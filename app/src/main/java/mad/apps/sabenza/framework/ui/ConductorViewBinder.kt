...
private fun <T, V : View> required(id: Int, finder: T.(Int) -> View?)
        = LazyBinding { t: T, desc -> t.finder(id) as V? ?: viewNotFound(id, desc) }

@Suppress("UNCHECKED_CAST")
private fun <T, V : View> optional(id: Int, finder: T.(Int) -> View?)
        = LazyBinding { t: T, _ ->  t.finder(id) as V? }

@Suppress("UNCHECKED_CAST")
private fun <T, V : View> required(ids: IntArray, finder: T.(Int) -> View?)
        = LazyBinding { t: T, desc -> ids.map { t.finder(it) as V? ?: viewNotFound(it, desc) } }

@Suppress("UNCHECKED_CAST")
private fun <T, V : View> optional(ids: IntArray, finder: T.(Int) -> View?)
        = LazyBinding { t: T, _ -> ids.map { t.finder(it) as V? }.filterNotNull() }

// Like Kotlin's lazy delegate but the initializer gets the target and metadata passed to it
private class LazyBinding<T, V>(private val initializer: (T, KProperty<*>) -> V) : ReadOnlyProperty<T, V> {
...
private object LiveBindings {
    ...
    private val bindingMap = WeakHashMap<Any, MutableCollection<LazyBinding<*, *>>>()
    ...
    fun <T> register(target: T, lazy: LazyBinding<T, *>) {
